...
To impute missing numeric variables, we use recursive partitioning. Categorical variables are imputed using multivariate imputation by chained equations (MICE).
...
Predictors:  `car_age` and `home_val` show a bi-modal distribution, with one peak at zero and a normal right tail.  This is to be expected with 'home_val' as those who do not have a home would return a zero value. The same is not obvious for why 'car_age' would have so many clustered closed to zero.
...
Based on the mosaic and box plots below, `sex` and `red_car` are both only weakly related to the response. This will be kept in mind during the variable selection phase.
...
We establish that there is only one pair of predictors that have a co-variance greater than 0.5": 'home_val' and 'income'.  We may consider combining into an interaction term, or possibly removing one from the model.  We also note that many of the correlations with the target variable appear low. This finding is consistent with the plots above.
...
Step 1. Although there is a 3:1 class imbalance in the response variable, models built with balanced training sets had AIC values higher than those that randomly sampled from the available data. This led us to proceed without taking steps to balance the training set.
...
This model includes all predictors and selects variables that reduce the Akaike information criterion (AIC).
...
***Dispersion***
We assess dispersion with two calculations whose results are shown below.
...
We note that linearity is questionable for `home-kids`, but not convincing enough to remove at this time.
...
Examining the standardized residuals (.std.resid) and the Cook’s distance (.cooksd) using `broom::augment()`, we note:
-Cooks distance indicates several standout observations (3722, 3592, 6501) but no influential points (id. D >1.0)
-There are no observations with std residual greater than 3 stdev - ie., no influential observations.
...
Each point on the below plot represents an aggregation of the prediction & residual values for each percentile bin of the predictions.  We observe that the higher percentiles have a higher average residual. We see this as a pattern that suggests the model may be misclassified.
...
`yoj` and `homekids` are removed because they are not significant predictors.
...
Establish new variable of liquidity = (home_val+1)/(income+1) to account for the co-variance that was discovered earlier.  Some `home_val` observations are = 0, likely indicating renters, so +1 is added to the variables to avoid division by zero.
...
The models do well at predicting no crashes but perform less well at predicting crashes  with a .5 threshold. Given the payout risk, a threshold  near 0.3 might be advisable.

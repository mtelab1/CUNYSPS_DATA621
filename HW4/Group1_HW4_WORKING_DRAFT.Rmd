...
library(knitr)
library(ggthemes)
library(mgcViz)#beta reg viz tool
set_flextable_defaults(table.layout = 'autofit', digits = 2)
```
...
```{r preserve_df}
#preserve cleaned dataframe w/ all variables
dfpreserve = df
```
...
m1pred = predict(model1,df_test,type= 'response')
summ(model1)
```
...
print(paste0('Next we obtain a Chi-Squared test statistic of ', devchi ,  ' This communicates that the null hypothesis is not rejected and their are no problems with dispersion.'))
```
...
To assess any impact of influential observations on our model results we examined both the standardized residuals (.std.resid) and the Cook’s distance (.cooksd). While the latter distinguished several notable outliers (3722, 3592, 6501), these data were not influential (D >1). This is consistent with our plot of standardized residuals (below) - with no observations exeeding 3 standard deviations. 
...
```{r set_flagdf2_test , message = FALSE}
#create df to contain transformed sqrt, log, vars 
trans_dftest<-df_test
trans_dftest%<>%
    mutate(log_bluebook= log(bluebook))%>%
    mutate(sqrt_income=sqrt(income))%>%
    dplyr::select(!c(bluebook, income))
...
m2pred = predict(model2,trans_dftest,type= 'response')
summ(model2)
```
\newpage
...
```{r flagm2_diagnostics, include = FALSE}
#additional metrics
...
```{r set_flagdf3vars_test,include=FALSE}
...
int_dftest %<>% mutate(clm_freq=cut(clm_freq, breaks=c(-Inf, 0, 3, Inf), labels=c("none","moderate","high")))
```
...
m3pred = predict(model3,int_dftest,type= 'response')
summ(model3)
```
...
```{r flagm3_diagnostics, include=FALSE}
#additional metrics
...
```{r}
options(width = 40)
t = t(as.matrix(rbind(model1_metrics, model2_metrics, model3_metrics)))
trainresults = data.frame(t, row.names = rownames(t))
trainresults <- data.frame(metric = row.names(t), t)
names(trainresults) = slice(trainresults, 1)
trainresults <- slice(trainresults, -1)
trainresults = trainresults%>%rename(metric = model)%>%flextable()

trainresults = colformat_double(trainresults,big.mark=",", digits = 3, na_str = "N/A")
trainresults = set_caption(trainresults ,'Training Set Performance')
trainresults = set_table_properties(trainresults, layout = "fixed", width = 0)
trainresults
```
...
```{r}
testresults = cbind(df_test,m1pred,m2pred,m3pred)
testresults = testresults %>% 
        mutate(m1pred = as.factor(case_when(m1pred>= 0.5 ~ 1,m1pred < 0.5 ~ 0)))%>% 
        mutate(m2pred = as.factor(case_when(m2pred>= 0.5 ~ 1,m2pred < 0.5 ~ 0)))%>% 
        mutate(m3pred = as.factor(case_when(m3pred>= 0.5 ~ 1,m3pred < 0.5 ~ 0)))
fig = set_caption(tidy(confusionMatrix(data = testresults$target_flag, reference = testresults$m1pred))[c(1,3)]%>%data.frame() %>%
  merge(tidy(confusionMatrix(data = testresults$target_flag, reference = testresults$m2pred))[c(1,3)]%>%data.frame(), by = 'term')%>%
  merge(tidy(confusionMatrix(data = testresults$target_flag, reference = testresults$m3pred))[c(1,3)]%>%data.frame())%>%
  setNames(c('metric','model1','model2','model3'))%>%
  flextable(),'Test Set Performance')
fig = colformat_double(fig,
...
```{r cost_model_saturated,fig.show="hold"}
...
```{r cost_model_1, fig.show="hold"}
dfcrashm2 = dm2(dfcrash)
...
```
\newpage
Now with a log transformation on the response.  We can see that much of the linearity we noticed above is non-existant.
```{r, fig.height=7, fig.align= 'center', fig.show="hold"}
df_num = dfcrash %>% select(where(is.numeric),-target_amt) %>% names()
dfcrash %>%
  gather(df_num, key = "var", value = "value") %>%
...
Lastly, lets review with the below predictor transformations instead.
-`sqrt(income)`
-`log(bluebook)`
-`travtime^2`
...
```{r, fig.height=7, fig.show="hold"}
dfcrashm3 = dm3(dfcrash)
df_num = dfcrashm3 %>% select(where(is.numeric),-target_amt) %>% names()
dfcrashm3%>%
  gather(df_num, key = "var", value = "value") %>%
  ggplot(aes(x = value, y = target_amt))+
  geom_point() +
...
```{r cost_model_4, fig.show="hold"}
library(MASS)
costm3b = rlm(target_amt ~ .,dfcrashm3, 
             method = "MM",
              weights = wts
              )
colformat_double(tidy(costm3b)%>%flextable(),
  big.mark=",", digits = 2, na_str = "N/A")
colformat_double(glance(costm3b)%>%flextable(),
  big.mark=",", digits = 2, na_str = "N/A")
...
dm5 = function(df){
  df = df%>%
   ...
   mutate(mvr_pts = cut(mvr_pts, breaks = c(-Inf, 0, 3, Inf), labels=c("none", "low", "high")))%>%
   mutate(tif = cut(tif, breaks = c(0, 5, 10, Inf), labels=c("low", "moderate", "high")))%>%
   mutate(clm_freq=cut(clm_freq, breaks=c(-Inf, 0, 3, Inf), labels=c("none","moderate","high")))%>%
   mutate(kidsdriv = case_when(kidsdriv == 0 ~ 'N' ,TRUE  ~ 'Y'))%>%
   mutate(job = case_when(job == 'Blue Collar' ~ 'Blue Collar',job != 'Blue Collar' ~ 'Professional', TRUE ~    as.character(NA)))%>%
   mutate_if(is_character, ~(factor(.)))%>%
   mutate(education = factor(education, levels=c("High School", "Bachelors", "Masters", "PhD")))
  return(df)
}
```
...
```{r}
detach("package:MASS", unload = TRUE)
dfcrashm5 = dm5(dfcrash)%>%
  mutate(income = sqrt(income),travtime = travtime**2)%>% 
  mutate(severity = pmin(1,target_amt/bluebook ))%>%
  select(-target_amt)%>%
  filter(severity<1)
```
...
samplelm = lm(severity ~ ., dfcrashm5)
weightlm = glm(samplelm$residuals ~ samplelm$fitted.values)
wts = (sqrt(weightlm$fitted.values*-1)) +abs(min(weightlm$fitted.values*-1))
#+ abs(min(weightlm$fitted.values))
plot(samplelm$fitted.values, wts)
```
To apply the beta distribution we use a general additive model(GAM); which gives us the option of adding non-linear variables.  After some find tuning; we find that adding the 'log(bluebook)' and 'income' as non-linear predictors improves our models.  
```{r cost_model_5, include= FALSE}
costm5 = lm(severity ~ ., dfcrashm5)
```

```{r cost_model_5b , include=FALSE}
costm5null <- gam(severity ~ mstatus +sex +mstatus +sex +education +travtime +car_use + bluebook +tif +car_type +oldclaim +clm_freq +revoked +mvr_pts +car_age +home_val +yoj +income 
               , wieghts = wts
               ,family=betar(link="logit")
               , data=dfcrashm5)
costm5b <- gam(severity ~ mstatus +sex +mstatus +sex +education +car_use + s(log(bluebook)) +tif +car_type +oldclaim +clm_freq +revoked +mvr_pts  +home_val +yoj + s(income)
               +travtime
               +income
               +car_age
               +oldclaim
...
print(m5sum$family)
cbind.data.frame(
residual.df = m5sum$residual.df,
r.sq = m5sum$r.sq,
dispersion = m5sum$dispersion,
dev.expl = m5sum$dev.expl
)%>%flextable()
```
...
v1 = m5sum$p.table
v1 = data.frame(Terms = row.names( v1), v1)
fig1 = v1%>%flextable()%>%colformat_double(big.mark=",", digits = 2, na_str = "N/A")
fig1 = set_caption(fig1,'Parametric Terms')
fig1

v2 = m5sum$s.table
v2 = data.frame(Terms = row.names( v2), v2)
fig2 = v2%>%flextable()%>%colformat_double(big.mark=",", digits = 2, na_str = "N/A")
fig2 = set_caption(fig2,'Non-Parametric Terms')
fig2
...
```{r }
set_caption(anova(costm5null,costm5b, test = 'Chisq')%>%flextable(),'We applied an anova test to confirm the changes we made from the base model are significant to the model')
```
...
We approached the former using logistic regression to assign a probability of crash; and letting a designated threshold bucket the observations into crash vs no-crash.  In reviewing the distribtion of the variables and their relationship to the target, we opted to transform a selection of the variables into factors based on the grouping behavior.  Through additional transformations and feature engineering, we landed on a model that minimized AUC and AIC.
After building the model to bucket the observations into the binary groups, we moved onto constructing second model to predict the cost implications.  We were able to make slight improvements through feature selections and tranformations, but the low $R^2$ warrented additional experimenting.  Prompted by the distribution of the residuals; we applied WLS, Robust Regression, and IWLS.  The jump in $R^2$ was encouraging.  However, it appeared that 'bluebook' variable was carrying all of the models.  The decision was made to rethink the target variable as a ratio 'bluebook' and regression on a [0,1] severity using the beta function.


```{r initial_cleaning_, include = FALSE}
raw = test
# initial clean of col names
raw%<>%clean_names
# remove any empty rows and cols
...
```{r include = FALSE}
df$target_flag = predict(model3,type =  'response',df)
df$target_flag = case_when(
        df$target_flag >= 0.5 ~ 1,
        df$target_flag < 0.5 ~ 0)
df$target_amt = predict.gam(costm5b,family=betar(link="logit"),type = 'response', df)*df$bluebook
write.csv(df, 'Car Insurance Assignment Predictions')
```

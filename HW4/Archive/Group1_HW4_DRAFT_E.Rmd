...
```{r, , echo=FALSE, message = FALSE, fig.show="hold", out.height="50%"}
...
ggarrange(plotlist=b_plots, height = 1.5, ncol = 1)
...
```
```{r plot covariance, fig.height=8, fig.width=8}
df%>%plot_correlate() 
```
...
print(paste0('We divide the deviance by the residuals to obtaine the value ', devres,'. There is no overt concern since the values is not greater than 1'))
...
print(paste0('Next we obtain a Pearson Chi-Squared test statistic of ', devchi ,  ' This communcates that the null hypothesis is not rejected and their are no problems with dispersion.'))
```

***Assumption of Linearity***

We can note that linearity is questionable for home-kids, but not convincing enough to remove at this time.
yoj, oldclaim, or income are not considered in this diagnositic since they were not significant. 
...
```{r flagm1_Outliers_Influenctial , echo= c(2,3)}
...
```{r flagm2_diagnostics , fig.align='center', fig.height=10,fig.width=10, message = FALSE, echo=c(3,4)}
...
```{r flagm3_diagnostics , fig.align='center', fig.height=5,fig.width=5, message = FALSE, echo = c(3,4)}
...
By removing some of the variables we earmarked earlier in the analysis, we can see a reduction in the Residual Standard Error.
...
We attempt to correct the heteroscadasticity of the residual plots through transformations.
...
```
Now with a log transformation on the response.  We can see that this evaporates much of the linearity we noticed above.
```{r, fig.height=9}
df_num = dfcrash %>% select(where(is.numeric),-target_amt) %>% names()
dfcrash %>%
  gather(df_num, key = "var", value = "value") %>%
  ...
  ggthemes::theme_fivethirtyeight()
```
Below is another set of plots that feature the below predictor transformations.  We can interate through additional attempts, but it appears that the relationship 
sqrt: income
log: bluebook
quadratic: travtime
```{r, fig.height=9}
...
Apply
For our third model, we move to include weights.  The first step shown below is the calculation of the weights.  Our strategy is to use the results from our base model; regressing the residuals against its fitted values.  We end up with a distribution of values which loosely represent the variance.  By taking the absolute value of this regression; we can place less value on the observations with greater variance.

Below is a plot of how the weights along the scale of the response variable.
```{r set_wieghts,fig.align = 'center', echo = 3}

...
print(paste0('We confirm these results using the Chi-Square test:pchisq(deviance, degrees of freedom) =', devchi ,  ' This value is not low enough to reject the null hypothesis that there are no problems with dispersion.'))
```

***Assumption of Linearity***

We can note that linearity is questionable for home-kids, but not convincing enough to remove at this time.
yoj, oldclaim, or income are not considered in this diagnostic since they were not significant. 
...
We now review how the fitted logistic regression compares to the data using the below marginal plots.
...
Establish new variable of liquidity = (home_val+1)/(income+1) to account for the co-variance that was discovered earlier.  Some home_val observations are = 0, likely indicating renters, so +1 is added to the variables to avoid 0 division NaN results.
...
```{r cost_model_5}
costm5null <- gam(scale ~ mstatus +sex +mstatus +sex +education +travtime +car_use + bluebook +tif +car_type +oldclaim +clm_freq +revoked +mvr_pts +car_age +home_val +yoj +income 
               , wieghts = wts
               ,family=betar(link="logit")
               , data=dfcrashm5)
costm5b <- gam(scale ~ mstatus +sex +mstatus +sex +education +car_use + s(log(bluebook)) +tif +car_type +oldclaim +clm_freq +revoked +mvr_pts  +home_val +yoj + s(income)
               +travtime
               +income
               +car_age
               +oldclaim
...
summary(costm5b)
plot(costm5b$fitted.values,dfcrashm5$scale)
plot(costm5b$fitted.values, dfcrashm5$scale - costm5b$fitted.values)
qqPlot(dfcrashm5$scale - costm5b$fitted.values)
```

```{r}
anova(costm5null,costm5b, test = 'Chisq')
```
...
```{r testdataclean, include = FALSE}
testsetprep = function(raw){
raw%<>%clean_names
...
#######
# df$car_age[df$car_age < 0] <- NA
# car_age<-imputate_na(df, car_age, method = "rpart", seed = 999)
# home_val<-imputate_na(df, home_val, method = "rpart")
# income<-imputate_na(df, income,  method = "rpart")
# age<-imputate_na(df, age, method = "rpart")

#build a working dataframe
temp<-cbind(car_age, home_val, yoj, income, age)
temp%<>%as.data.frame(temp)
df%<>%dplyr::select(!c(car_age, home_val, yoj, income, age))%>%cbind(temp)
df%<>%mutate_if(is.numeric, round)
...
```{r flagdatatransformation}
flagproc = function(raw){
...
# identify unique values in our character co
...
flagsubmission = flagproc(test)
costsubmission = testsetprep(test)
costsubmission = dm5(test)
```

```{r}
preds = predict(model3,flagsubmission,type="response")
submission$target_flag = case_when(preds>= 0.5 ~ 1,preds < 0.5 ~ 0)
submission$target_amt = predict(costm5b,flagsubmission,type="response")*flagsubmission$bluebook
```
